...
double compute_max_error_advection_field(int r, int M, int N, double *u, int ldu) {
  int i, j;
  double err = 0.0;
  double t = r * dt;
  for (i=0; i < M; i++) {
...
__global__
void update_north_south_boundary(int N, int M, double *u, int ldu) {
  for (int j=1; j < N+1; j++) { //top and bottom halo
    u[j] = u[M * ldu + j];
    u[(M + 1) * ldu + j] = u[ldu + j];
  }
...
__global__
void update_east_west_boundary(int M, int N, double *u, int ldu) {
  for (int i=0; i < M+2; i++) { //left and right sides of halo
    u[i * ldu] = u[i * ldu + N];
    u[i * ldu + N + 1] = u[i * ldu + 1];
  }
...
__global__
void update_advection_field_kernel(int M, int N, double *u, int ldu, double *v, int ldv,
			double Ux, double Uy) {
  update_advection_field(M, N, u, ldu, v, ldv, Ux, Uy);
}

__global__
void copy_field_kernel(int M, int N, double *u, int ldu, double *v, int ldv) {
...
void run_serial_advection_device(int M, int N, int reps, double *u, int ldu) {
  ...
  for (int r = 0; r < reps; r++) {
    ...
    update_advection_field_kernel <<<1,1>>> (M, N, &u[ldu + 1], ldu, &v[ldv + 1], ldv,
				  Ux, Uy);
