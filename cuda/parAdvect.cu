...
__global__ void opt_update_advection_field_kernel(int M, int N, double *u,
  ...
  if (g_x < M && g_y < N) {
    ...
    // update value
    V(v, g_x + 1, g_y + 1) =
        cim1 * (cjm1 * V(shared_u, x, y) + cj0 * V(shared_u, x, y + 1) +
                cjp1 * V(shared_u, x, y + 2)) +
        ci0 * (cjm1 * V(shared_u, x + 1, y) + cj0 * V(shared_u, x + 1, y + 1) +
               cjp1 * V(shared_u, x + 1, y + 2)) +
        cip1 * (cjm1 * V(shared_u, x + 2, y) + cj0 * V(shared_u, x + 2, y + 1) +
                cjp1 * V(shared_u, x + 2, y + 2));
  }
}
